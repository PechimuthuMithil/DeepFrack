# this file has approach with partial products -> thus inputs always from DRAM and outputs always cached
        ...
        for Q in range(2**q): # Iterating over all possible Weight Caching Patterns (WCP)
            ...
            # FOR MIDDLE LAYERS
                ...
                for layer in range(1,n-1):
                    outputs_size = (tile)**2 * num_parallel[start+layer]
                    inputs_size = (tile)**2 * num_parallel[start+layer]
                    ...
                    # for attn and qkv inputs = weights so weights always cached with inputs
                ...
                if (ChosenCached[n-1] == '1' or layer_names[end]=='Attn' or layer_names[end]=='QKV'):
                    ...
                    const = np.dot(mask,np.array([[weights_size,],[inputs_size,],[outputs_size,]]))*factor['SLC']
                    if np.all(const >= arch_sizes):
                        # print(f'Out at end')
                        CurrAnswer = float('inf')
                    else:
